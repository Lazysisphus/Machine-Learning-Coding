...
def calc_H_D(trainLabelArr):
    ...
    # 遍历该集合就可以遍历所有出现过的标签并计算其Ck
    # 这么做有一个很重要的原因：首先假设一个背景，当前标签集中有一些标签已经没有了，比如说标签集中
    # 没有0，这是很正常的，说明当前分支不存在这个标签
    # 式5.7中有一项Ck，那按照式中的针对不同标签k
    # 计算Ck和D并求和时，由于没有0，那么C0=0，此时C0/D=0，log2(C0/D)=log2(0)，出现了问题
...
def calcH_D_A(trainDataArr_DevFeature, trainLabelArr):
    ...
    # 初始为0
    H_D_A = 0
    # 获得当前特征A在数据集D上所有可能的取值
    trainDataSet = set([label for label in trainDataArr_DevFeature])

    # 对于每一个特征取值遍历计算经验条件熵的每一项
    for i in trainDataSet:
        # 计算H(D|A)
        # trainDataArr_DevFeature[trainDataArr_DevFeature == i].size / trainDataArr_DevFeature.size，即|Di|/|D|
        # calc_H_D(trainLabelArr[trainDataArr_DevFeature == i])，即H(Di)
...
def calcBestFeature(trainDataList, trainLabelList):
    ...
    # 例如 a = np.array([1, 2, 3]); b = np.array([1, 2, 3]).T
    # 若不转置，a[0] = [1, 2, 3]，转置后b[0] = 1, b[1] = 2
    # 对于标签集来说，能够很方便地取到每一位是很重要的
    trainDataArr = np.array(trainDataList) # (60000, 784)
    trainLabelArr = np.array(trainLabelList).T # (60000, )

    # 获取当前特征数目，也就是数据集的横轴大小，784
    ...
    for feature in range(featureNum):
        ...
        # np.array(trainDataArr[:, feature].flat): 再转换成一条竖着的矩阵，大小为60000*1
        # 只是初始是这么大，运行过程中是依据当前数据集大小动态变的
...
def getSubDataArr(trainDataArr, trainLabelArr, A, a):
    ...
    for i in range(len(trainDataArr)):
        ...
        if trainDataArr[i][A] == a:
            # 那么将该样本的第A个特征切割掉，然后将切割后的样本放入返回的数据集中
...
def createTree(*dataSet):
    ...
    # 之所以使用元组作为参数，是由于后续递归调用时数据集需要对某个特征进行切割，在函数递归
    ...
    classDict = {i for i in trainLabelList} # set类型
    # 如果D中所有实例属于同一类Ck，则置T为单个节点，并将Ck作为该节点的类，返回T
    # 即若所有样本的标签一致，也就不需要再分化，返回标签作为该节点的值，返回后这就是一个叶子节点
    if len(classDict) == 1:
        # 因为所有样本都是一致的，在标签集中随便拿一个标签返回都行，这里用的第0个
        # 因为你并不知道当前标签集的长度是多少，但运行中所有标签只要有长度都会有第0位
        return trainLabelList[0]

    # 如果A为空集，则置T为单个节点，并将D中实例数最大的类Ck作为该节点的类，返回T
    # 即如果已经没有特征可以用来再分化了，就返回占大多数的类别
    if len(trainDataList[0]) == 0:
        # 返回当前标签集中占数目最大的标签
        return majorClass(trainLabelList)

    # 否则，按式5.10计算A中个特征值的信息增益，选择信息增益最大的特征Ag，及其对应的信息增益EpsilonGet
    ...
    # 否则，对Ag的每一可能值ai，依Ag=ai将D分割为若干非空子集Di
    # 将Di中实例数最大的类作为标记，构建子节点，由节点及其子节点构成树T，返回T
    treeDict = {Ag:{}} # Ag表示特征的idx
    # 特征值为0时，进入0分支
