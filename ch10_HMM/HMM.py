...
def trainParameter(fileName):
    ...
    # 每个字只有四种状态，所以下面初始化时状态相关参数均为4
    # 初始化PI的一维数组，因为对应四种状态，大小为4
    PI = np.zeros(4)
    # 初始化状态转移矩阵A，涉及到四种状态各自到四种状态的转移，所以大小为4x4
    A = np.zeros((4, 4))
    # 初始化观测概率矩阵，分别为四种状态到每个字的发射概率
    # 因为是中文分词，使用ord(汉字)即可找到其对应编码
    # 这里用一个65536的空间来保证对于所有的汉字都能找到对应的位置来存储
    B = np.zeros((4, 65536))
    # 去读训练文本
    ...
    # A依据10.37
    # B依据10.38
    # 注：并没有使用Baum-Welch算法，只是借助了其内部的三个参数生成公式
    # 其实公式并不是Baum-Welch特有的，只是在那一节正好有描述
    for line in fr.readlines():
        ...
        #       例如：
        #           PI = [0， 0， 0， 0]，当本行第一个字是B，即表示开头时，PI中B对应位置为0，
        ...
        for i in range(len(curLine)):
            ...
            # 在整行的状态链中添加该单词的状态链
            # wordLabel是列表类型，每个元素是一个标签，和预想的不太一样
    ...
    for i in range(len(PI)):
        ...
        # x大的时候，log也大，x小的时候，log也相应小
        # 我们最后比较的是不同概率的大小，所以使用log没有问题
...
def participle(artical, PI, A, B):
    ...
    for line in artical:
        # 初始化δ，δ存放四种状态的概率值
        # 因为状态链中每个状态都有四种概率值，因此长度时该行的长度
        delta = [[0 for i in range(4)] for i in range(len(line))] # 二维列表，[4*字数]
        # 依据算法10.5 第一步：初始化
        for i in range(4):
