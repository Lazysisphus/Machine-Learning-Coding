...
alpha0:0.3, mu0:-2.0, sigmod0:0.5, alpha1:0.7, mu1:0.4, sigmod1:1.0
----------------------------
'''
...
def loadData(mu0, sigma0, mu1, sigma1, alpha0, alpha1):
    ...
    # 初始化第一个高斯分布，生成数据
    # 数据长度为length * alpha系数，以此来满足alpha的作用
    # 即length * alpha0个样本服从第一个高斯分布，length * alpha1个样本服从第二个高斯分布
    data0 = np.random.normal(mu0, sigma0, int(length * alpha0)) # list类型
    # 第二个高斯分布的数据
    ...
    # 对总的数据集进行打乱
    # 其实不打乱也没事，只不过打乱一下直观上让人感觉已经混合了，读者可以将下面这句话屏蔽以后看看效果是否有差别
...
def EM_Train(dataSetList, iter = 500):
    ...
    :return: 估计的参数
    '''
    # 将可观测数据 y 转换为数组形式，主要是为了方便后续运算
    dataSetArr = np.array(dataSetList) # (1000, )
    ...
    while (step < iter):
        # 每次进入一次迭代后迭代次数加1
        step += 1
        # 步骤2：E步：依据当前模型参数，计算分模型 k 对观测数据 y 的响应度
