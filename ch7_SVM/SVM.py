...
class SVM:
    ...
    def __init__(self, trainDataList, trainLabelList, sigma = 10, C = 200, toler = 0.001):
        ...
        注:
        关于这些参数的初始值：
            参数的初始值大部分没有强要求，请参照书中给的参考
            例如C是调和间隔与误分类点的系数，在选值时通过经验法依据结果来动态调整
            本程序中的初始值参考于《机器学习实战》中SVM章节，因为书中也使用了该数据集，只不过抽取了很少的数据测试，参数在一定程度上有参考性
            如果使用的是其他数据集且结果不太好，强烈建议重新通读所有参数所在的公式进行修改
            例如在核函数中σ的值高度依赖样本特征值范围，特征值范围较大时若不相应增大σ会导致所有计算得到的核函数均为0
        '''
        self.trainDataMat = np.mat(trainDataList)       # 训练数据集-(1000, 784)
        self.trainLabelMat = np.mat(trainLabelList).T   # 训练标签集，为了方便后续运算提前做了转置，变为列向量-(1000, 1)
        ...
        self.E = [0 for _ in range(self.trainLabelMat.shape[0])] # SMO运算过程中的Ei，初始值为包含1000个0的列表
    ...
    def calcKernel(self):
        ...
        for i in range(self.m):
            ...
            # 得到式7.90中的x
            X = self.trainDataMat[i, :] # (1, 784)
            # 小循环遍历Xj，Xj为式7.90中的Z
            # 由于 Xi * Xj 等于 Xj * Xi
            # 一次计算得到的结果可以同时放在k[i][j]和k[j][i]中，这样一个矩阵只需要计算一半即可
            # 所以小循环直接从i开始
            for j in range(i, self.m):
                # 获得Z
                Z = self.trainDataMat[j, :] # (1, 784)
                # 先计算||X - Z||^2
                result = (X - Z) * (X - Z).T
    ...
    def isSatisfyKKT(self, i):
        ...
        # 式7.111到7.113，判断条件中考虑到了松弛变量，和书上有不同
        # ***此处有空再理解一下***
        # 依据7.111
    ...
    def calc_gxi(self, i):
        ...
        # 但是读者应该有发现，在“7.2.3 支持向量”开头第一句话有说到
        # “对应于α>0的样本点(xi, yi)的实例xi称为支持向量”，也就是说只有支持向量的α是大于0的
        # 在求和式内的对应的αi*yi*K(xi, xj)不为0，非支持向量的αi*yi*K(xi, xj)必为0，也就不需要参与到计算中
        # 也就是说，在g(xi)内部求和式的运算中，只需要计算α>0的部分，其余部分可忽略
        # 因为支持向量的数量是比较少的，这样可以再很大程度上节约时间
        # 从另一角度看，抛掉支持向量的概念，如果α为0，αi*yi*K(xi, xj)本身也必为0
        # 从数学角度上将也可以扔掉不算
    ...
    def getAlphaJ(self, E1, i):
        ...
        # 这一步是一个优化性的算法
        # 实际上书上算法中初始时每一个Ei应当都为-yi（因为初始α、b为0，则g(xi)初始必然为0）
        # 然后每次按照书中第二步去计算不同的E2来使得|E1-E2|最大，但是时间耗费太长了
        # 作者最初是全部按照书中编写，但是本函数运行需要3秒左右，所以进行了一些优化措施
        # --------------------------------------------------
        # 在Ei的初始化中，由于所有α为0，所以一开始是设置Ei初始值为-yi
        # 但是这里修改为与α一致，初始状态所有Ei为0，在运行过程中再逐步更新
        ...
        # 在程序运行后期后其实绝大部分Ei都已经更新完毕了
        # 下方优化算法只不过是在程序运行的前半程进行了时间的加速，在程序后期其实与未优化的情况无异
        # ------------------------------------------------------
    ...
    def train(self, iter = 100):
        ...
        while (iterStep < iter) and (parameterChanged > 0):
            ...
            for i in range(self.m):
                # 查看顺次的第一个变量是否满足KKT条件，如果不满足则作为SMO中第一个变量从而进行优化
