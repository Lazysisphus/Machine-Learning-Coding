...
备注：
    对于mnist而言，李航的统计学习方法中有一些关键细节没有阐述，
    建议先阅读原作者的个人博客，其中有详细阐述，阅读结束后再看该程序。
...
class maxEnt:
    ...
    def __init__(self, trainDataList, trainLabelList, testDataList, testLabelList):
        ...
        self.n = 0                                  # 训练集中（xi，y）对种类数，即特征函数fi数目
        self.M = 2500                               # 不太明白这个量的意义，https://www.zhihu.com/question/266997179?sort=created
    ...
    # 计算特征函数f(x, y)关于模型P(X|Y)与经验分布P_(X)的期望值
    def calcEpxy(self):
        '''
        计算特征函数f(x, y)关于模型P(Y|X)与经验分布P_(X)的期望值
        P后带下划线“_”表示P上方的横线，程序中部分下划线表示“|”，部分表示上方横线，请根据具体公式自行判断
        即“6.2.2 最大熵模型的定义”中第二个期望（97页第二个期望）
        :return:
        '''
        # 初始化期望存放列表，对于每一个xy对都有一个期望
        # 这里的x是单个的特征，不是一个样本的全部特征。例如x={x1，x2，x3.....，xk}，实际上是（x1，y），（x2，y）等
        # 但是在存放过程中需要将不同特征的分开存放，李航的书可能是为了公式的泛化性高一点，所以没有对这部分提及
        # 具体可以看原作者博客，里面有详细介绍：www.pkudodo.com
        Epxy = [0] * self.n
        # 对于每一样本进行遍历
        for i in range(self.N):
            ...
            Pwxy = [0] * 2
            # 计算P(y = 0 | X)
            # 注：程序中X表示是一个样本的全部特征，x表示单个特征，这里表示前者
    ...
    # 计算特征函数f(x, y)关于经验分布P_(Y|X)的期望值
    def calcEp_xy(self):
        '''
        计算特征函数f(x, y)关于经验分布P_(x, y)的期望值
        下划线表示P上方的横线，同理Ep_xy中的“_”也表示p上方的横线
        即“6.2.2 最大熵的定义”中第一个期望（97页下方那个式子）
        :return: 计算得到的Ep_xy
        '''
        # 初始化Ep_xy列表，长度为n
        Ep_xy = [0] * self.n
    ...
    # 创建基于训练集的特征查询字典，将每个出现过的特征与一个idx绑定
    def createSearchDict(self):
        ...
        # 所以在生成字典的时总共生成了特征数个字典，例如在mnist中样本有784维特征，所以生成784个字典
        # 属于不同特征的xy存入不同特征内的字典中，使其不会混淆
    ...
    # 统计各种(xi, y)对数目，以及(xi, y)种类数，即特征函数fi的数目
    def calc_fixy(self):
        '''
        计算(x, y)在训练集中出现过的次数
        :return:
        '''
        # 有多少个特征，就建立多少个字典，属于不同特征的(x, y)对存入不同的字典中，保证不被混淆
        ...
        for i in range(len(self.trainDataList)):
            ...
            for j in range(self.featureNum):
                # 将出现过的(x, y)对放入字典中并计数值加1，例如，第0个特征的对应字典保存有{(0, 0): 46, (0, 1): 4, ...}
    ...
    def calcPwy_x(self, X, y):
        ...
        :param y: 该样本的标签，本程序使用二分类，只有两种标签0和1
        :return: 计算得到的Pw(Y|X)
        '''
        # 分子
        numerator = 0
        ...
        for i in range(self.featureNum):
            ...
            if (X[i], y) in self.xy2idDict[i]:
                ...
                # 由于当(x, y)存在时fi(x，y)为1，因为(x, y)对肯定存在，所以直接就是1
                # 对于分子来说，就是n个wi累加，最后再指数就可以了
                # 因为有n个w，所以通过id将w与(x, y)绑定，前文的两个搜索字典中的id就是用在这里
                numerator += self.w[index]
            # 同时计算其他一种标签y时候的分子
            # 下面的z并不是全部的分母，再加上上式的分子以后才是完整的分母，即z = z + numerator
...
if __name__ == '__main__':
    ...
    # 初始化最大熵类
    # 20000个样本运行时间太长，本项目中取50个训练样本，重点学习代码
    maxEnt = maxEnt(trainData[:50], trainLabel[:50], testData, testLabel)
