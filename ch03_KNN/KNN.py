...
def calcDist(x1, x2):
    '''
    计算两个样本点向量之间的距离
    使用的是欧氏距离，即对样本点每个元素相减的平方求和，再对和开方
    :param x1:向量1
    :param x2:向量2
    :return:向量之间的欧式距离
    '''
...
def getClosest(trainDataMat, trainLabelMat, x, topK):
    '''
    预测样本x的标记
    先找到与样本x最近的topK个点，并查看它们的标签，然后选取多数类的标签作为当前预测的标签
    ...
    for i in range(len(trainDataMat)):
        cur_x = trainDataMat[i]
        curDist = calcDist(cur_x, x)
        distList[i] = curDist
    ...
    # array返回的是整个索引值列表，我们通过[:topK]取列表中前topk个放入list中
    # ----------------优化点-------------------
    # 由于我们只取topK小的元素索引值，所以其实不需要对整个列表进行排序，argsort是对整个列表进行排序的，存在时间上的浪费
    # 字典有现成的方法可以只排序topk大或topk小，可以自行查阅，对代码进行稍稍修改即可
...
def model_test(trainDataArr, trainLabelArr, testDataArr, testLabelArr, topK):
    ...
    errorCnt = 0
    # 遍历测试集，对每个测试集样本进行测试
    # 由于计算向量与向量之间的时间耗费太大，测试集有6000个样本，所以这里人为改成了测试200个样本点
