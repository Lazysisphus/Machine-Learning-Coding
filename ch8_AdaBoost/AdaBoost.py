...
运行结果：
    ...
    iter:0:10, sigle error:0.0804, finall error:0.0804
    iter:1:10, sigle error:0.1448, finall error:0.0804
    iter:2:10, sigle error:0.1362, finall error:0.0585
    iter:3:10, sigle error:0.1864, finall error:0.0667
    iter:4:10, sigle error:0.2249, finall error:0.0474
    iter:5:10, sigle error:0.2634, finall error:0.0437
    iter:6:10, sigle error:0.2626, finall error:0.0377
    iter:7:10, sigle error:0.2935, finall error:0.0361
    iter:8:10, sigle error:0.3230, finall error:0.0333
    iter:9:10, sigle error:0.3034, finall error:0.0361
'''
...
def calc_e_Gx(trainDataArr, trainLabelArr, n, div, rule, D):
    ...
    # 初始化分类误差率为0
    e = 0
    # 将训练数据矩阵中特征为n的那一列单独拿出来做成数组
    # 直接对庞大的训练集进行操作的话会很慢
    x = trainDataArr[:, n] # (10000, )
    # 同样将标签也转换成数组格式，x和y的转换只是单纯为了提高运行速度
    # 测试过相对直接操作而言性能提升很大
    y = trainLabelArr # (10000, )
    predict = []
    ...
    for i in range(trainDataArr.shape[0]):
        if x[i] < div:
            ...
            if y[i] != H: 
                e += D[i]
    # 返回预测结果和分类错误率e
    # 预测结果其实是为了后面做准备的，在算法8.1（2）第四步式8.4中exp内部有个Gx，要用在那个地方
    # 以此来更新新的D
...
def createBosstingTree(trainDataList, trainLabelList, treeNum = 50):
    ...
    '''
    # 将数据和标签转化为数组形式
    trainDataArr = np.array(trainDataList) # (10000, 784)
    trainLabelArr = np.array(trainLabelList) # (10000, )
    # 每增加一层数后，当前最终预测结果列表
    ...
    for i in range(treeNum):
        ...
        # 依据式8.4更新D
        # 考虑到该式每次只更新D中的一个w
        # 为了简化程序，下面不使用循环的方式更新D中每一个w
        # 而是以向量相乘的形式，使用一个式子将所有w全部更新完
        # 该式需要线性代数基础，如果不太熟练建议补充相关知识
        # 当然，单独更新w也没有问题
...
if __name__ == '__main__':
    ...
    # 原作者用了40层树模型，这里为了减少计算时间，用10层树模型
    tree = createBosstingTree(trainDataList[:10000], trainLabelList[:10000], 10)
