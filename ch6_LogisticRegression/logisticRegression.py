...
def loadData(fileName):
    ...
    for line in fr.readlines():
        ...
        # 二项逻辑斯蒂克用于解决二分类问题
        # Mnsit有0-9十个标记，由于是二分类任务，所以将标记0的作为1，其余为0
        ...
        if int(curLine[0]) == 0:
            ...
            labelList.append(0)
        # 存放标记
        # [int(num) for num in curLine[1:]] -> 遍历每一行中除了以第一个元素（标记），并将所有元素转换成int类型
...
def logisticRegression(trainDataList, trainLabelList, iter = 200):
    ...
    # 将数据集由列表转换为矩阵形式，主要是后期涉及到向量的运算，统一转换成矩阵形式比较方便
    ...
    for i in range(iter):
        ...
        for j in range(trainDataList.shape[0]):
            ...
            # 但是似然函数由于有求和项，并不能直接对w求导得出最优w
            # 需要针对似然函数求和部分中每一项进行单独地求导w，得到针对该样本的梯度，并进行梯度上升
            # 因为是要求似然函数的极大值，所以是梯度上升，如果是极小值就梯度下降，梯度上升是加号，下降是减号
            # 求和式中每一项单独对w求导结果为：xi * yi - (exp(w * xi) * xi) / (1 + exp(w * xi))
            # 如果对于该求导式有疑问可查看原作者的博客：www.pkudodo.com

            # 计算w * xi，因为后式中要计算两次该值，为了节约时间这里提前算出
            # 其实也可直接算出exp(wxi)，为了读者能看得方便一点就这么写了，包括yi和xi都提前列出了
            wxi = np.dot(w, trainDataList[j])
            yi = trainLabelList[j]
            xi = trainDataList[j]
            # 梯度上升
            w +=  h * (xi * yi - (np.exp(wxi) * xi) / ( 1 + np.exp(wxi)))
